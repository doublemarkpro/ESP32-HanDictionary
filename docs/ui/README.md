...
> 下文 `tab5-*.png` 是视觉与交互基准，不是固件截图。UI 第一轮已实现专用 LVGL 页面；`rendered/*.png` 才是同一份产品页面代码在电脑上实际渲染的截图。硬件服务由模拟器替代，真机验收尚未进行。
...
1. 已实现统一页面容器、主页、返回、查字与基础工具页面；先在真机验证显示/触摸、联网、麦克风、扬声器和 SD。
2. 补齐合法可用的字典与教学音频内容，并扩展中文字体和笔顺。
3. 完善家长设置、课程编辑、计时历史、多闹钟、在线天气和录音回放。
4. 扩展语音操作与视觉细节；涉及修改计时/提醒的语音指令需确认流程。
