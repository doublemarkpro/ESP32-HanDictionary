...
2026-09-14 增加电池详情弹窗，点击状态栏电池图标即可显示百分比、电压、带符号电流和
瞬时功率。百分比改为按单节 3.0–4.15 V 的分段放电曲线估算；INA226 不是库仑计，
2000 mAh 标称容量不直接进入算法。字典 Legacy/P4X 变体关闭未使用的 USB-A 与外接 5 V
输出，关屏时维持 Wi-Fi 低功耗状态。深度睡眠与 RX8130 定时唤醒尚未实现，一天闹钟守候
续航仍需单独设计和真机放电验收。
