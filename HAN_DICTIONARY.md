...
本版为加入字体与 UI 使用了专用 `partitions/han16m.csv`：两个 6 MiB OTA 应用分区，并为当前约 2.85 MiB 的 assets 保留约 3.88 MiB。**从旧固件升级时，首次应通过 USB 完整烧录（包含分区表和 assets），不能只 OTA 替换应用。** 先备份重要设备设置，确认型号与串口后运行：

增量 OTA 可作为后续的下载优化：服务器按设备当前版本生成差分包，设备应用差分后仍将完整的新固件写入未运行的 OTA 槽。因此它能减少流量和更新时间，但不能让应用超过 6 MiB 分区，也不能替代双槽回滚。本项目继续按完整镜像大小执行容量检查；ESP32-P4 不采用仅支持 ESP32-C2/C3 的 `esp_bootloader_plus` 差分方案。

已有 `xiaozhi.bin` 时，让电脑或手机与 Tab5 连接同一局域网，在浏览器访问设置页
SSID 下显示的设备 IPv4 地址（例如 `http://192.168.5.131/`），即可上传固件并写入备用
OTA 槽。网页 OTA 只更新应用分区，不会更新分区表或独立的 `assets` 语音模型资源；这些内容
发生变化时仍需通过 USB 完整烧录。该页面只适合可信局域网中的开发测试，不应通过路由器
端口转发暴露到互联网；生产发布应使用 HTTPS 和签名固件。
