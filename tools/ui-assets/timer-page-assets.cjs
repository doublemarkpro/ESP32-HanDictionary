...
async function build() {
  ...
  const specs = [
    ['han_font_timer', 28,
      '语文数学英语今日作业已完成进行中未开始有记录本周用时今天共分钟暂停开始继续完成本科准备正在专注周一二三四五六日不足网络与存储显示声音连接读取状态手机配网模式可使用本地功能卡内容字典音插画资源屏幕亮度播放自动锁立即关长按唤醒再向上滑动解正在切换器启服务器端口订阅主题留言板还没有收到最新刷新设置重新从配置账号登录客户端失败断开等待家庭匿名密码保存最近条外观浅深色自动时段夜间开启早晨关闭并应用取消计划拼查找键盘全部轻声退格清空输入例如离线库中索引筛选同汉笔排序整理候选结果对应点击查看左右滑动翻页可按调缩小范围任务繁忙请稍后重试缺更新包完成后点或直接选择会示这里释义组词结构笔画播放顺一步完整，。“”·Ⅱ▶✓—…'],
