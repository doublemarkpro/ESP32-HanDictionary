...
test('timetable reuses twelve small icons, not a full-page bitmap', async () => {
  let bytes = 0;
  const source = fs.readFileSync(path.join(root, 'main/han_dictionary/assets/timetable_assets.c'), 'utf8');
  for (const name of ['book', 'calculator', 'science', 'art', 'sport', 'computer', 'music',
    'martial', 'labor', 'flute', 'star', 'backpack']) {
