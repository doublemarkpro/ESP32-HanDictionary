...
test('timetable uses compact generated PNG subject icons, not a full-page bitmap', async () => {
  let bytes = 0;
  const source = fs.readFileSync(path.join(root, 'main/han_dictionary/assets/timetable_assets.c'), 'utf8');
  for (const name of ['book', 'calculator', 'english', 'science', 'art', 'sport', 'computer',
    'music', 'martial', 'labor', 'flute', 'star', 'club']) {
    const png = fs.readFileSync(path.join(root, `assets/graphics/timetable/${name}.png`));
    const m = await sharp(png).metadata();
    assert.deepEqual([m.width, m.height], [48, 48]);
    assert.ok(m.hasAlpha);
    assert.ok(source.includes(`{${Array.from(png).join(',')}}`));
    bytes += png.length;
  }
  assert.ok(bytes < 160 * 1024);
});
