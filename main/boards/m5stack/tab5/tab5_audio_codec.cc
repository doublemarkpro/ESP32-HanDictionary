...
Tab5AudioCodec::Tab5AudioCodec(void* i2c_master_handle, int input_sample_rate,
                               int output_sample_rate, gpio_num_t mclk, gpio_num_t bclk,
                               gpio_num_t ws, gpio_num_t dout, gpio_num_t din, gpio_num_t pa_pin,
                               uint8_t es8388_addr, uint8_t es7210_addr, bool input_reference) {
    duplex_ = true;                              // 是否双工
...
void Tab5AudioCodec::CreateDuplexChannels(gpio_num_t mclk, gpio_num_t bclk, gpio_num_t ws,
...
void Tab5AudioCodec::EnableInput(bool enable) {
    ...
    if (enable) {
        ...
        if (input_reference_) {
            // ES7210 I2S TDM slots are MIC1, MIC3, MIC2, MIC4 (datasheet Fig. 2e).
            // Slot 1 is MIC3/AEC_P from HPOUT_L; slot 2 is the second microphone.
...
int Tab5AudioCodec::Read(int16_t* dest, int samples) {
    ...
    const int result = esp_codec_dev_read(input_dev_, dest, samples * sizeof(int16_t));
    if (result != ESP_CODEC_DEV_OK) {
        ESP_LOGW(TAG, "Audio read failed: %d", result);
        return 0;
    }
    return samples;
}
